...
use crate::{Result, FFStudioError};

pub fn get_data_dir() -> Result<PathBuf> {
    if let Some(proj_dirs) = ProjectDirs::from("com", "FAI", "FFStudio") {
        let dir = proj_dirs.data_dir().to_path_buf();
        fs::create_dir_all(&dir)
            .map_err(|e| FFStudioError::file_system(format!("Failed to create data directory: {}", e)))?;
        Ok(dir)
    } else {
        Err(FFStudioError::file_system("Could not determine application data directory"))
    }
}
...
pub fn init_workdir() -> Result<()> {
    ...
    if !workflow_path.exists() {
        fs::create_dir_all(&workflow_path)
            .map_err(|e| FFStudioError::file_system(format!("Failed to create workflows directory: {}", e)))?;
        log::info!("Created workflows directory: {:?}", workflow_path);
    }

    if !cache_path.exists() {
        fs::create_dir_all(&cache_path)
            .map_err(|e| FFStudioError::file_system(format!("Failed to create cache directory: {}", e)))?;
        log::info!("Created cache directory: {:?}", cache_path);
    }

    if !tmp_path.exists() {
        fs::create_dir_all(&tmp_path)
            .map_err(|e| FFStudioError::file_system(format!("Failed to create tmp directory: {}", e)))?;
...
pub fn load_nodes(ffmpeg_path: &str) -> Result<Vec<Node>> {
    ...
    if !full_path.exists() {
        return Err(FFStudioError::file_system(format!("Cache file not found for FFmpeg path: {}", ffmpeg_path)));
    }
    ...
    log::debug!("Loaded {} nodes from cache for FFmpeg path: {}", nodes.len(), ffmpeg_path);
    Ok(nodes)
}
...
pub fn save_nodes(ffmpeg_path: &str, nodes: &Vec<Node>) -> Result<()> {
    ...
    log::debug!("Saved {} nodes to cache for FFmpeg path: {}", nodes.len(), ffmpeg_path);
    Ok(())
}