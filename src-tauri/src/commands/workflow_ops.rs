...
use crate::utils::filesystem::{get_data_dir, save_nodes, load_nodes};
use crate::workflow::types::{WorkflowStructure, Node, Response};
use crate::workflow::manager::{get_workflow_list};
use crate::{Result, FFStudioError, log_error, to_user_message};


pub fn add_workflow(name: String, path: String, env: String, desc: String, nodes: &Vec<Node>, version_data: &Vec<String>) -> Result<()> {
    ...
    std::fs::write(&wf_full_path, json_data)
        .map_err(|e| FFStudioError::file_system(format!("Failed to save workflow '{}': {}", name, e)))?;
...
pub async fn get_nodes_request(window: Window, ffmpeg_path: String) -> Result<()> {
    ...
    match load_nodes(&ffmpeg_path) {
        Ok(nodes) => {
            log::info!("Successfully loaded {} nodes for FFmpeg path: {}", nodes.len(), ffmpeg_path);
...
pub async fn get_workflow(window: Window, name: String) -> Result<()> {
    if name.trim().is_empty() {
        ...
        let _ = window.emit("get_workflow_listener", 
            Response {
    ...
    let nodes = match load_nodes(&workflow.path) {
        Ok(nodes) => {
            log::info!("Loaded {} cached nodes for workflow '{}'", nodes.len(), name);
            nodes
        }
        Err(_) => {
            ...
            match parse_ffmpeg(&workflow.path, &workflow.env) {
                Ok(nodes) => {
                    ...
                    log::info!("Parsed and cached {} nodes for workflow '{}'", nodes.len(), name);
                    nodes
                }
                Err(e) => {
                    let error = FFStudioError::from(e);
...
#[tauri::command]
pub async fn create_workflow(window: Window, name: String, path: String, env: String, desc: String) -> Result<()> {
    // Validate inputs
    if name.trim().is_empty() {
        let error_msg = "Workflow name cannot be empty";
        log_error(&FFStudioError::workflow(error_msg), "create_workflow");
        let _ = window.emit("create_workflow_listener", Response {
            message: error_msg.to_string(),
            build: "".to_string(),
            version: "".to_string(),
            nodes: Vec::new(),
    ...
    // Save the workflow
    if let Err(e) = add_workflow(name.clone(), path.clone(), env.clone(), desc.clone(), &nodes, &version_data) {
    ...
    let _ = window.emit("create_workflow_listener",
        Response {
            message: response_message.to_string(),
...
pub async fn delete_workflow(name: String) -> Result<()> {
    ...
    std::fs::remove_file(&wf_full_path)
        .map_err(|e| FFStudioError::file_system(format!("Failed to delete workflow '{}': {}", name, e)))?;
...
#[tauri::command]
pub async fn edit_workflow(_window: Window, name: String, path: String, env: String, desc: String) -> Result<()> {
    if name.trim().is_empty() {
        return Err(FFStudioError::workflow("Workflow name cannot be empty"));
    }
    ...
    if !wf_full_path.exists() {
        return Err(FFStudioError::workflow(format!("Workflow '{}' not found", name)));
    }

    let data = std::fs::read_to_string(&wf_full_path)
        .map_err(|e| FFStudioError::file_system(format!("Failed to read workflow '{}': {}", name, e)))?;
